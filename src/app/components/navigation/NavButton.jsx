    ...
    <div className='absolute cursor-pointer z-50' style={{ transform: `translate(${x}, ${y})` }}>
      ...
      >
        <span className='relative w-20 h-20 p-4 animate-spin-slow-reverse group-hover:pause hover:text-accent'>
          ...
          <div className="mt-6 -ml-0 text-foreground text-lg font-medium text-center">{label}</div>        </span>
      </NavLink>
...
// 'use client';
// import { Github, Home, Linkedin, Palette, Phone, Twitter, User, NotebookText, Trophy, HandCoins, AppWindow } from 'lucide-react';
// import Link from 'next/link';
// import React, { useEffect, useState } from 'react';
// import { motion } from 'framer-motion';
...
//         href={link}
//         target={newTab ? '_blank' : '_self'}
//         className="text-foreground rounded-full flex flex-col items-center justify-center custom-bg"
//         aria-label={label}
//         name={label}
//         onClick={handleClick}
//       >
...
//           {getIcon(icon)}
//           <span className="peer bg-transparent absolute top-0 left-0 w-full h-full" />
//           {/* Optional: Keep tooltip for hover if desired */}
//           <span className="absolute hidden peer-hover:block px-2 py-1 left-full mx-2 top-1/2 -translate-y-1/2 bg-background text-foreground text-sm rounded-md shadow-lg whitespace-nowrap">
//             {label}
//           </span>
//         </span>
...
//       </NavLink>
//       {/* <div className="mt-2 text-foreground text-sm font-medium">{label}</div> */}
//     </div>
//   );
// };
